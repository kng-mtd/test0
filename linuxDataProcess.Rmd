...
- **簡単な検索 → `grep`（BRE）**
- **複雑な検索・置換 → `grep -E`（ERE）、`sed -E`**
- **高度なテキスト処理 → `awk`**
- **より強力な正規表現 → `grep -P`（Perl互換）**

## **正規表現のメタ文字と違い（基礎正規表現 vs 拡張正規表現）**  

| **正規表現の種類** | **使用するコマンド** | **オプション** |
|------------------|----------------|------------|
| **基礎正規表現（BRE）** | `grep`、`sed` | デフォルト |
| **拡張正規表現（ERE）** | `grep -E`、`sed -E`、`awk` | `-E`（grep, sed） |

---

### **1. メタ文字一覧**
| メタ文字 | **意味** | **基礎正規表現（BRE）** | **拡張正規表現（ERE）** |
|----------|---------|------------------|------------------|
| `.` | 任意の1文字 | ○ | ○ |
| `^` | 行の先頭 | ○ | ○ |
| `$` | 行の末尾 | ○ | ○ |
...
### **2. 基礎正規表現（BRE: Basic Regular Expression）**
#### **使用するコマンド**
- `grep` （`grep -E` は不要）
- `sed`
- `awk`（基本的には ERE を使用）
...
#### **例**
```bash
grep 'hello' file.txt  # "hello" を含む行を検索
grep '^start' file.txt  # "start" で始まる行を検索
grep 'end$' file.txt  # "end" で終わる行を検索
grep '[0-9]' file.txt  # 数字を含む行を検索
```

**注意: `{}` や `+`, `|` などは `\` を付ける必要がある**
```bash
grep '\(hello\)' file.txt  # "hello" をグループ化
grep '\{3\}' file.txt  # 直前の文字が3回出現
```

---

### **3. 拡張正規表現（ERE: Extended Regular Expression）**
#### **使用するコマンド**
- `grep -E`
- `sed -E`
- `awk`（デフォルトでERE）
...
#### **例**
```bash
grep -E 'hello|world' file.txt  # "hello" または "world" を検索
grep -E '[0-9]{3,5}' file.txt  # 数字が3～5回連続する行を検索
grep -E 'A+BC?' file.txt  # "A" が1回以上、"B" が1回、"C" は0または1回
```

**ポイント: `\` を付ける必要がない**
```bash
grep -E '(hello)' file.txt  # グループ化（BREでは \( \) が必要）
grep -E '{3}' file.txt  # 直前の文字が3回出現
```
...
| 関数      | 説明                                       | 使用例                                |
|-----------|--------------------------------------------|---------------------------------------|
| `add`     | 数値の合計を計算                           | `[1, 2, 3] | add` → `6`              |
| `length`  | 配列や文字列の長さを取得                   | `[1, 2, 3] | length` → `3`            |
| `max`     | 配列の最大値を取得                         | `[1, 2, 3] | max` → `3`               |
| `min`     | 配列の最小値を取得                         | `[1, 2, 3] | min` → `1`               |
| `sum`     | 配列の合計値を計算                         | `[1, 2, 3] | sum` → `6`              |
| `group_by`| 配列を指定したキーでグループ化             | `group_by(.key)`                     |
| `select`  | 条件に一致する要素を抽出                   | `select(.age > 25)`                  |
| `unique`  | 重複を取り除いてユニークな要素を取得       | `[1, 2, 2, 3] | unique` → `[1, 2, 3]` |
| `map`     | 各要素に関数を適用して新しい配列を作成     | `map(. * 2)`                         |
| `flatten` | 配列をフラット化                           | `[[1, 2], [3, 4]] | flatten` → `[1, 2, 3, 4]` |
...
## **5. `xmlstarlet` で Wikipedia の XML を解析する**
Wikipedia の XML ファイルを `xmlstarlet` で解析する例を紹介します。

### **(1) 記事タイトルを一覧表示**
```sh
xmlstarlet sel -t -m "//page/title" -v "." -n enwiki-latest-pages-articles.xml
```
→ Wikipedia のすべての記事タイトルを取得。

---

### **(2) 特定の記事の本文を取得**
```sh
xmlstarlet sel -t -m "//page[title='Python (programming language)']/revision/text" -v "." -n enwiki-latest-pages-articles.xml
```
→ 「Python (programming language)」の記事の本文を取得。

---

### **(3) 記事のリビジョン ID を取得**
```sh
xmlstarlet sel -t -m "//page[title='Python (programming language)']/revision/id" -v "." -n enwiki-latest-pages-articles.xml
```
→ 最新のリビジョン ID を取得。
...
## **`xmlstarlet` を使って逐次的に CSV に追加する方法**

`xmlstarlet` を使って CSV ファイルに1行ずつ追加するためには、以下のコマンドを使用します。

```sh
xmlstarlet sel -T -t -m "//page" \
  -v "id" -o "," \
  -v "title" -o "," \
  -v "revision/text" -n \
  enwiki-latest-pages-articles.xml >> wikipedia_pages.csv
```

- `>> wikipedia_pages.csv` を使うことで、既存の CSV ファイルに追記（追加）されます。
- これを使えば、ファイル全体を一度に読み込まずに、各ページの情報を1行ずつ逐次的に追加することができます。
...
```
id,title,text
12345,Example Page,{{Infobox example}}
67890,Python (programming language),'''Python''' is a programming language...
23456,History of the world,The history of the world is the history of humanity...
```
...
```sh
# see xml file and in case <mediawiki xmlns="http://www.mediawiki.org/xml/export-0.11/"

# add header
echo 'id,title,text' > enwiki1.csv

xmlstarlet sel -T -N mw="http://www.mediawiki.org/xml/export-0.11/" -t -m "//mw:page" -o "\"" -v "mw:id" -o "\"," -v "mw:title" -o "\"," -v "normalize-space(mw:revision/mw:text)" -o "\"" -n enwiki1.xml > enwiki1.csv
```
...
このコマンドを実行すると、`xml0.xml` ファイルの中から、各ページの `id`、`title`、`text` をカンマ区切りで抽出した CSV が出力されます。出力例としては次のような形になります：
